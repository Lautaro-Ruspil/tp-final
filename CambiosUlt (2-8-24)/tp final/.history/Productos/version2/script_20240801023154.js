...
document.addEventListener('click' , (e) =>{
    let re = e.target.dataset.index;
    let product2 = products.find(product => product.id == re)
    console.log(product2.id);
    let button = document.querySelector(`[ button data-index="${product2.id}" ]`);
    console.log(button);
})
...
let total = 0 ;
    ...
    function renderProducts() {
        ...
        for(let i = 0 ; i < products.length ; i++){
            ...
            productElement.innerHTML += `
                ...
                <input data-index = "${product.id}" type= "number" class= "input" name="cantidad" placeholder="Cantidad" min="0" max="${product.stock}">
                <button onclick="addToCart(${i})"${product.stock === 0 ? 
                    'disabled' : ''} data-index = " ${product.id} " class= "button">
                    Agregar al Carrito
                </button>
            `
    ...
    function renderCart() {
        ...
        let total1 = 0;
            function controlStock(products){
            let cantidad = document.querySelectorAll(".input").value;
            parseInt(cantidad);
            console.log(typeof cantidad);
            if(cantidad > 0 && products.stock >= 0){
                total1 += products.price * cantidad;
                console.log(total1);
            }
        }
        /*let arrayproducts = [];
