...
let total = 0 ;
    ...
    function renderProducts() {
        ...
        for(let i = 0 ; i < products.length ; i++){
            ...
            productElement.innerHTML += `
                ...
                <p id="${product.id}" hidden></p><p id="${product.id}" hidden></p>
                <input  type= "number" id="input" class= "input" name="cantidad" placeholder="Cantidad" min="0" max="${product.stock}">
                <button onclick= "addToCart(${i})" ${product.stock === 0 ? 'disabled' : ''} 
                    id= "button" class="button" data-index="${product} ">
                    Agregar al Carrito
                </button>
            `
            productsElements.appendChild(productElement);
            let input = document.querySelectorAll(".cantidad");
            console.log(input);
        }
    ...
    function addToCart(productsIndex){
        ...
        let cantidad = document.querySelectorAll(".input");
