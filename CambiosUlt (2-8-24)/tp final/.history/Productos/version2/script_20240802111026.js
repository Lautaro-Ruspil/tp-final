...
function cargarClick() {
    ...
    botones.forEach(element => {
        element.addEventListener("click", (e) => {
            ...
            console.log(products[e.target.id - 1]);
...
function addToCart(productsIndex, cantidad) {
    ...
    if (productsIndex.stock > 0) {
        cart.push({ name: productsIndex.name, price: productsIndex.price, id: productsIndex.id })
...
/*
 //inicializar la lista de productos
    ...
    let pru=document.querySelector("#prueba").addEventListener("keypress",()=>{
        console.log(prueba.value);

    })


    function filtrar(arrInputs,par2) {
     let encontrado;
     arrInputs.forEach(element => {
        // (i => i.getAttribute("data-index")  == productsIndex)
         if(element.getAttribute("data-index")  == par2){
