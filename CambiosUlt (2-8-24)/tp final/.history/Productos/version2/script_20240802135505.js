...
function renderProducts() {
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        productElement.innerHTML += `
                ...
                <input type="number" class="input" id="cantidad-producto-${product.id}" placeholder="Cantidad" min="0" max="${product.stock}">
...
function cargarClick(){
    ...
    botones.forEach(element => {
        element.addEventListener("click", (e) => {
            let productId = parseInt(e.target.id - 1);
            console.log(productId);
            let cantInput = document.querySelector(`#cantidad-producto-${productId}`);
            console.log(parseInt(cantInput.value));

            let input = e.target.previousElementSibling.value;
            addToCart(products[e.target.id - 1],cantInput)
        })
...
function addToCart(productsIndex, cantidad) {
    ...
    if ((cantidad > 0 && productsIndex.stock >= cantidad)){
        ...
        if(productInCart){
            ...
            productInCart.cantidadComprada += cantidad;
        }else{
...
function renderCart() {
    ...
    for(let i = 0 ; i < cart.length ; i++){
        ...
        let texto = document.createTextNode(`${items.name} - ${items.price} x ${items.cantidadComprada}`);
