...
let total = 0 ;
        ...
        for(let i = 0 ; i < products.length ; i++){
            ...
            productElement.innerHTML += `
                ...
                <button onclick= "addToCart(${i})" ${product.stock === 0 ? 'disabled' : ''} 
                    id= "button" class="button" data-index="${product.id}">
                    Agregar al Carrito
                </button>
            `
    ...
    //let input = document.getElementById(`"#${productElement}"`);
    //console.log(input);
    ...
    function addToCart(productsIndex){
        ...
        let input = cantidad.find(i => cantidad.getAttribute("data-index")  == productsIndex)
        console.log(input);
        cantidad[productsIndex].getAttribute("data-index");
