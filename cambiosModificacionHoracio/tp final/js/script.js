...
//variable  arreglo carrito 
let cart = [];
//Total General 
let total = document.querySelector("#total");
//variable para inicializada en 0 para guardar
...
function renderProducts() {
    ...
    for (let i = 0; i < products.length; i++) {
        ...
        productElement.innerHTML += `
                <img src="../imagenes/${product.id}.png">
...
inputs.forEach(element => {
    element.addEventListener('keyup', (e) => {
        ...
        // Convertir el valor a número y asegurarse de que sea un valor válido
        let quantity = parseInt(e.target.value) || 0;

        // Validar y ajustar la cantidad según el stock disponible
        if (quantity > productsIndex.stock) {
            e.target.value = productsIndex.stock;
        } else if (quantity < 0) {
            e.target.value = 0;
        } else {
...
function renderCart() {
    ...
    if (cart.length === 0){
        ...
        messageCart.textContent = "Tu carrito esta vacío"
        messageCart.classList.add("messageCart");
        cartElements.appendChild(messageCart);
        return; //Salir de la funcion si el carrito esta vacio
    } else {
        ...
        cart.forEach(items => {
            ...
            itemElements.innerHTML = ` 
                <img src="../imagenes/${items.id}.png">
                <span class="spanNombre">Descripción: ${items.name} - $${items.price} x ${items.cantidadComprada} Unidades = $${items.price * items.cantidadComprada}</span>
                <button class="btnDevolver" id="${items.id}">Devolver Producto</button>
                `
...
renderCart(); //renderizo el carrito para mostrar el mensaje (carrito vacio)

//funcion para devolver productos
function returnProduct(){
    let buttonsReturn = document.querySelectorAll(".btnDevolver")
    
    buttonsReturn.forEach(button => {
        button.addEventListener('click' , (e) => {
            let productId = parseInt(e.target.id)
            let productInCart = cart.find(items => items.id === productId);

            if (productInCart){
                let productsIndex = products.find(product => product.id === productId);

                //Descuento del total Gral
                totalCalculado -= productInCart.price * productInCart.cantidadComprada;
                total.innerHTML = totalCalculado;

                //Actualizar el stock del producto
                productsIndex.stock += productInCart.cantidadComprada;

                //Eliminar el producto del carrito
                cart = cart.filter(item => item.id !== productId);

                //Renderizar nuevamente productos y carrito
                renderProducts();
                renderCart();
            }
        });
